            ...
            {all_students.map((student, index) => (
              <View key={index} style={styles.studentChartContainer}>
                <View
                  ...
                  <Text style = {{fontSize : 15,fontWeight : "bold" , marginRight :10}}>{student.roll_no}</Text>
               <Text style = {{fontSize : 15,fontWeight : "bold" , marginRight :10}}>
               {student.name}
               </Text>
                </View>
                <View style={{ flex: 1, flexDirection: 'row' }}>
                  <TouchableOpacity style={this.state.presentPressedList.includes(index)
                        ? [styles.presentButton, { backgroundColor: 'green' }]
                        : styles.presentButton} onPress = {() => {
                  ...
                  <TouchableOpacity style={                     this.state.absentPressedList.includes(index)
...
const styles = StyleSheet.create({
 container: {
  ...
  footer: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgb(48, 148, 176)',
    marginTop:10,  
  },
});
